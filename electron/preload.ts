...
const electronAPI = {
  ...
  // New definitions to match type file
  onQueuesCleared: (callback: () => void) => {
    ipcRenderer.on("queues-cleared", callback);
    return () => {
      ipcRenderer.removeListener("queues-cleared", callback);
    };
  },
  onInitialStart: (callback: () => void) => {
    ipcRenderer.on(PROCESSING_EVENTS.INITIAL_START, callback);
    return () => {
      ipcRenderer.removeListener(PROCESSING_EVENTS.INITIAL_START, callback);
    };
  },
  getSolution: (screenshotPaths: string[]) => 
    ipcRenderer.invoke("get-solution", screenshotPaths),
  cragTextQuery: (query: string) =>
    ipcRenderer.invoke("crag-text-query", query),
  getDebugSolution: (data: {
    screenshotPaths: string[];
    previousSolution: string;
    debugScreenshotPaths: string[];
  }) => ipcRenderer.invoke("get-debug-solution", data),
  setView: (view: "queue" | "solutions" | "debug") => 
    ipcRenderer.invoke("set-view", view),
}
